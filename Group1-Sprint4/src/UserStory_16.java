...
public class UserStory_16{
	...
	public static String maleLastNames(List<List<String>> idList, List<List<String>> faList){
			...
			for(int j = 0; j < children.size(); j++){
				if(SearchFunc.checkMale(children.get(j), idList)){
					if(!SearchFunc.fetchSurname(children.get(j), idList).equals(familyName)){
						res = "ERROR: FAMILY: US16: " + faList.get(i).get(0) + ": This family's male members have different last name: Son '" + children.get(j) + "' has a last name which is different from his father's.";
