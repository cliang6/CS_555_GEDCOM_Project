...
public class UserStory07_Original{
	...
	public static String checkLessThan150ForIndi(List<List<String>> idList, List<List<String>> faList){
		try{
			...
			if(!idList.get(i).get(3).equals("NA") && !idList.get(i).get(6).equals("NA") && idList.get(i).get(5).equals("False")){
				//int alive;
				// int birthYear, birthMon, birthDay;
				// int deathYear, deathMon, deathDay;
				String [] birthNextLine = idList.get(i).get(3).split("\\-") ;
				...
				if(deathYear - birthYear <= 150){
					...
					if(birthMon < deathMon){
						...
						if(birthDay < deathDay){
							...
							res = "ERROR: Individual:US07:I"+String.valueOf(i)+" More than 150 years old from "+idList.get(i).get(3)+ " to " +idList.get(i).get(6);
			...
			if (!idList.get(i).get(3).equals("NA") && !idList.get(i).get(6).equals("NA") &&  idList.get(i).get(5).equals("True")){
			...
			if(currentYear - birthYear <= 150){
					...
					if(birthMon < currentMonth){
						...
						if(birthDay < currentDay){
							...
							res = "ERROR: Individual:US07:I"+String.valueOf(i)+" More than 150 years old "+idList.get(i).get(3);
						        System.out.println(res);
						}
					}
					else{
